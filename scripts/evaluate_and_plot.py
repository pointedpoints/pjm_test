...
def run_evaluate_and_plot(config_path: str, split: str = "test") -> None:
    ...
    prediction_frames = {}
    metric_rows = []
    for prediction_file in sorted(directories["prediction_dir"].glob(f"*_{split}_seed*.parquet")):
    ...
    metrics_path = directories["metrics_dir"] / f"{split}_metrics.csv"
    ...
    pd.DataFrame(dm_rows).to_csv(directories["metrics_dir"] / f"{split}_dm.csv", index=False)
    plot_hourly_mae(prediction_frames, directories["plots_dir"] / f"{split}_hourly_mae.png")

    best_run = metrics_df.iloc[0]["run"]
    plot_high_volatility_week(prediction_frames[best_run], directories["plots_dir"] / f"{split}_high_vol_week.png")
