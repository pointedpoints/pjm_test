...
def _frame(*, split: str, days: int) -> pd.DataFrame:
    ...
    for index, ts in enumerate(timestamps):
        ...
        for quantile, offset in [(0.1, -3.0), (0.5, -2.0), (0.9, 1.0)]:
            rows.append(
                {
                    "ds": ts,
                    "y": y,
...
def test_evaluate_median_bias_grid_outputs_baseline_and_bounded_rows() -> None:
    ...
    assert {"raw_monotonic", "hour_cqr", "hour_median_bias_cap5", "hour_median_bias_cap10"} <= validation_variants
