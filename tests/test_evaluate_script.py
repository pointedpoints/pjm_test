...
def test_workspace_evaluate_discovers_prediction_files_without_filename_regex(tmp_path: Path) -> None:
    ...
    regime_df = pd.read_csv(workspace.artifacts.regime_metrics("test"))
    spike_df = pd.read_csv(workspace.artifacts.spike_score_diagnostics("test"))
    ...
    assert "custom_name_without_contract" in set(regime_df["run"])
    assert "custom_name_without_contract" in set(spike_df["run"])
