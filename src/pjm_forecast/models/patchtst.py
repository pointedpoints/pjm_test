...
def _build_patchtst_class() -> tuple[type[Any], type[Any]]:
    try:
        ...
        raise ImportError("PatchTSTModel requires neuralforecast and torch to be installed.") from exc
    return NeuralForecast, PatchTST
...
class PatchTSTModel(ForecastModel):
    ...
    def __post_init__(self) -> None:
        self._neuralforecast_cls, self._patchtst_cls = _build_patchtst_class()
    ...
    def fit(self, train_df: pd.DataFrame) -> None:
        ...
        for member_kwargs in self._resolved_member_kwargs():
            ...
            model = self._patchtst_cls(**member_kwargs, loss=self._build_loss(), valid_loss=self._build_valid_loss())
    ...
    def predict(self, history_df: pd.DataFrame, future_df: pd.DataFrame) -> pd.DataFrame:
        if not self._member_states:
            raise RuntimeError("PatchTSTModel must be fit before predict.")
        member_predictions = []
