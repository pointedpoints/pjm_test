...
def _build_tide_class() -> tuple[type[Any], type[Any]]:
    try:
        from neuralforecast import NeuralForecast  # type: ignore
        from neuralforecast.models import TiDE  # type: ignore
    except ImportError as exc:  # pragma: no cover - guarded by caller
        raise ImportError("TiDEModel requires neuralforecast and torch to be installed.") from exc
    return NeuralForecast, TiDE
...
class TiDEModel(ForecastModel):
    ...
    def __post_init__(self) -> None:
        self._neuralforecast_cls, self._tide_cls = _build_tide_class()
    ...
    def fit(self, train_df: pd.DataFrame) -> None:
        ...
        for member_kwargs in self._resolved_member_kwargs():
            ...
            model = self._tide_cls(**member_kwargs, loss=self._build_loss(), valid_loss=self._build_valid_loss())
    ...
    def predict(self, history_df: pd.DataFrame, future_df: pd.DataFrame) -> pd.DataFrame:
        if not self._member_states:
            raise RuntimeError("TiDEModel must be fit before predict.")
        member_predictions = []
    ...
    @classmethod
    def load(cls, path: Path) -> "TiDEModel":
